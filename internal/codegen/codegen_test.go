...
func TestGenerateBlankColumn(t *testing.T) {
	tables := []introspect.Table{
		{
			Schema: "public",
			Name:   "items",
			Columns: []introspect.Column{
				{Name: "_", DataType: "text", IsNullable: false, OrdinalPos: 1},
			},
		},
	}
...
// TestGenerateCompiles verifies that the output of Generate is valid, compilable
// Go source. It writes the generated file into a temporary module that replaces
// github.com/libliflin/gosq with a minimal local stub — no network required.
func TestGenerateCompiles(t *testing.T) {
	...
	src, err := Generate(tables, Config{Package: "schema", DotImport: true})
	if err != nil {
	...
	// Minimal gosq stub — satisfies the dot import without needing network access.
