...
- **Impact:** The contract for a column named `_` (single underscore) is now explicitly documented and tested. `toExported("_")` returns `"_"`, producing a field identifier like `Items_`. This was valid but untested — only the `___` (triple-underscore) case had coverage. A contributor adding logic to `toExported` now has an explicit test for the single-underscore input.

## Observed
- `TestToExported` had `{"___", "_"}` and `{"", "_"}` but not `{"_", "_"}`. The single-underscore case was implied but not explicit.
- `TestGenerateAllUnderscoreColumn` tested column `___` → `Items_`. Column `_` → `Items_` was noted in the testing guide as "valid but untested."
- `staticcheck ./...` was run and confirmed clean (no issues found).

## Applied
- Added `{"_", "_"}` to `TestToExported` (between `{"2fa_enabled", "_2faEnabled"}` and `{"___", "_"}`).
- Added `TestGenerateBlankColumn`: table `items`, column `_`, asserts full output including `Items_ = NewField("items._")`.
...
## Next
- The test suite now documents behavior for all known edge-case column name patterns. Further improvements should be driven by real user feedback — production schemas with non-public schema names, unusual column naming conventions, or scale issues beyond 17 tables.
